<template>
    <ui-model-edit model-name="pages" :model-id="$route.params.id" singular="Página" plural="Páginas" #default="{value, validate}">
        <ui-field label="Nome da página" layout="horizontal" :validate="validate.test('name')">
            <input type="text" class="form-control" v-model="value.name">
        </ui-field>

        <ui-field label="URL" layout="horizontal" :validate="validate.test('slug')">
        ...
        <ui-field label="Proprietário" layout="horizontal" :validate="validate.test('owner')">
            <ui-model-select model-name="user" v-model="value.owner"></ui-model-select>
        </ui-field>

        <ui-field label="Conteúdo" layout="horizontal" :validate="validate.test('content')">
